...
async function sendBookingCompletedWhatsApp(booking: BookingWithRelations) {
  const phone = booking.customer_profiles?.phone;
  if (!phone) return;

  const text = getBookingCompletedMessage(booking);
  ...
  try {
    ...
    console.error("Gagal mengirim WhatsApp selesai booking:", error);
  }
}
...
export function useUpdateBookingMutation(
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      } else if (variables?.input?.status === "done") {
        void sendBookingCompletedWhatsApp(data);
      }
      queryClient.invalidateQueries({ queryKey: bookingsKeys.all });
      queryClient.invalidateQueries({ queryKey: bookingsKeys.detail(data.id) });
